...
3. 책을 대여해본다.(`운영체제론`을 제외한 나머지를 대여해보기)
4. 3에서 대여한 책을 한 번 더 대여해본다.
5. 3에서 대여한 책을 반납한다.
6. 5에서 반납한 책을 다시 한 번 대여해본다.
7. 책 제목으로 검색해본다
...
1. 책 클래스
    ...
    - 메소드
        - `printInfo`: 책의 정보(책 번호, 제목, 저자, 대여 가능 여부)를 출력하는 기능
        - `getId`: 책 번호를 반환하는 기능
        - `getTitle`: 책 이름을 반환하는 기능
        - `getIsAvailable`: 대여 가능 여부를 반환하는 기능
        - `setIsAvailable`: 대여 가능 여부를 변경하는 기능
...
1. 방 클래스
    ...
    - 메소드
        ...
        - `printInfo`: 객실 정보(객실 번호, 객실 유형, 가격, 예약 가능 여부)를 출력하는 기능
        - `getRoomNumber`: 객실 번호를 반환하는 기능
        - `getRoomType`: 객실 유형을 반환하는 기능
        - `getIsAvailable`: 예약 가능 여부를 반환하는 기능
...
2. 고객 클래스
    ...
    - 속성
        ...
        - `email`: 고객 이메일
    - 메소드
        - `printInfo`: 고객 정보(ID, 이름, 이메일)를 출력하는 함수
3. 예약 클래스
    - 클래스명: `Booking`
    - 속성
        - `bookingId`: 예약 번호  
    ...
    - 메소드
        - `generateBookingId`: 난수를 반환하는 함수(난수 생성을 별도 함수로 분리할 경우에 사용하고, 함수로 분리하지 않으면 사용하지 않아도 됨)
        - `printInfo`: 예약 정보(예약 번호, 고객 정보, 객실 정보, 체크인/체크아웃 날짜)를 출력하는 기능
        - `getBookingId`: 예약 번호를 반환하는 기능
        - `getRoom`: 방 객체를 반환하는 기능
...
4. 호텔 클래스
    ...
    - 메소드
        ...
        - `findAvailableRoomByType`: 특정 객실 유형 중에 예약 가능한 객실을 출력하는 기능  
          예약 가능한 객실이 없다면 `예약 가능한 객실이 없습니다.`력
        - `makeBooking`: 예약하는 기능
